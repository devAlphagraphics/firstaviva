...
var headerHeight=99, footerHeight=595, contentMinHeight=640, popWinTextHeight=0;
...
$(function() {
    ...
    //==============All Pages
    $('#Header #Nav .bu').on('click',function() {
      switch ($(this).attr('id')) {
         case 'BU_Product':
            popMenuHandler('show');
            break;
         case 'BU_Form':
            $('html,body').animate({scrollTop: $('#Form').offset().top-headerHeight}, 800);
            break;
         case 'BU_Home':
             var source = getParameterByName('source');
             if(source === "")
                 window.location.href='/DMD201608/m/index.html';
             else
                 window.location.href='/DMD201608/m/index.html?source='+source;
            break;
         case 'Logo':
    ...
    // Menu
    $('#Mask #Win_Menu .bu').on('click',function() {
        var source = getParameterByName('source');
        if(source !== "")
            window.location.href = '/DMD201608/m/' + $(this).data('link')+'.html?source='+source;
        else
            window.location.href = '/DMD201608/m/' + $(this).data('link')+'.html';
    });
    $('#Mask #Win_Menu #BU_BackMenu').on('click',function() {
      popMenuHandler('hidden');
    });
    ...
    $('#Mask .popWin .text').css('height',(popWinTextHeight-130)+'px');
    $('#Mask #Win_TnC .text').css('height',(popWinTextHeight-80)+'px');

    $('#Mask .popWin .bu_close, #Mask .popWin #BU_Close').on('click',function() {
   ...
   $('#Form #BU_Submit').on('click',function() {
      if ($.trim($('#Form #FName').val())==='' ||
          typeof $('#Form input[name=FSex]:checked').val()==='undefined' ||
          ($.trim($('#Form #FMobile').val()))==='') {
             alert('姓名/性別/手機欄位為必填!!\r\n請檢查看看是否有欄位沒有資料.');
      } else {
         var isValid=1;
         ...
         if (isValid===1 && $.trim($('#Form #FEmail').val()).length>0) {
            if (ValidateEmail($.trim($('#Form #FEmail').val()))===false) {
               isValid=0;
               alert('Email資料格式錯誤, 請檢查輸入的Email資料!');
            }
         }
                ...
                $.ajax({
                    ...
                    success: function(res){
                        ...
                        if(res.code == 'success'){
                            ...
                            window.location.href=(source==='') ? "/DMD201608/m/Thankyou.html" : ("/DMD201608/m/Thankyou.html?source="+source);
                        }
             ...
             switch ($(this).attr('id')) {
                   case 'BU_More':
                      switch (toNumber(TestAns.score)) {
                          ...
                          case 70:
                          case 90:
                             window.location.href=(source === '') ? "/DMD201608/m/DM1601/index.html" : ("/DMD201608/m/DM1601/index.html?source="+source);
                             break;
                          case 40:
                          case 60:
                          case 80:
                          case 100:
                             window.location.href=(source === '') ? "/DMD201608/m/DM1602/index.html" : ("/DMD201608/m/DM1602/index.html?source="+source);
                             break;
                      }
                      break;
                   case 'BU_FBShare':
      ...
      default :
       //Sub Menu
          $('#FixdItem .menu li').on('click',function() {
             ...
             $(this).addClass('select');
             $('#DivContent .container > .page').hide();
             ...
             if ($(this).data('go')==='Try') {
                if (pageName==="index_1" || pageName==="index_2") {
                   $('#Try .step1 input[name=TYear]').prop('checked',false); //reset ans
                }
                $('#Try .step1 input[name=TSex]').prop('checked',false); //reset ans
                $('#Try #TAge').val('');
                $('#Try #TQuota').val('');
                $('#Try .step1').show();
                $('#Try .step2').hide();
          ...
          $('#Example #Content > div .item').on('click',function() {
             ...
             $('html,body').animate({scrollTop: $('#'+$(this).parent().attr('id')+'').offset().top-headerHeight}, 800);
          });
...
function popMenuHandler(status) {
   if (status==='show') {
      $('#Mask').css('top',headerHeight+'px');
      $('#Mask > *').hide();
      $('#Mask').fadeIn('fast');
      $('#Mask #Win_Menu').show();
...
function getCurentPageName() {
    ...
    console.log(arrPage);
    if(arrPage[2] === 'index.html' || arrPage[2] === ''){
        returnPage = 'index';
    }
    else if(arrPage[2] === 'Thankyou.html'){
        returnPage = 'index_4';
    }
    else if(arrPage[2] == 'DM1601'){
        returnPage = 'index_1';
    }
    else if(arrPage[2] == 'DM1602'){
        returnPage = 'index_2';
    }
    else if(arrPage[2] == 'DM1603'){
        returnPage = 'index_3';
    }
    else if(arrPage[2] == 'DM1604'){
        returnPage = 'index_5';
    }
